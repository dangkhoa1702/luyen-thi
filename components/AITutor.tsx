...
async function mockFetchAnswer({ question, subject, grade, intent, forbidFullSolution }:{ question:string; subject:Subject; grade:string; intent:QAIntent; forbidFullSolution:boolean }): Promise<AnswerPayload>{
  ...
  // FIX: Simplified the logic to handle only "Toán" and "Tiếng Anh" subjects, removing the "Ngữ văn" case which caused a type error.
...
function suggestionsBySubject(s: Subject){
  ...
  // FIX: Removed the "Ngữ văn" case as it is no longer a valid subject, allowing the default return to handle "Tiếng Anh".
  return [
